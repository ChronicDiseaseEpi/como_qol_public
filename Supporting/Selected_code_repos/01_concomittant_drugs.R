...
rxnorm_bnf <- read_csv("../Drug_lookups/bnf_rxnorm_atc_codes.zip")
...
trial_indic_drug <- read_csv("../selected_trials_in_respository_sas_with_medicine_condition.csv")

# Identify indication and medicine, all conditions have a code,
# except one Lilly file which does not belong
...
rev(a)
# 7      5      4      3      1 
# 328331 651503  58326  16363   1076 
rev(100*a %>%  prop.table()) %>%  cumsum() %>% round(1)
# 7     5     4     3     1 
# 31.1  92.8  98.3  99.9 100.0 
sum(is.na(conmed$atc_code))
...
############ Define comorbidities based on concomitant medications
...
## Save processed data
saveRDS(conmed_all, "Scratch_data/conmed_defined_comorbidity.Rds")

## Examine numbers for irregular definitions ----
# conmed_all <- readRDS("Scratch_data/conmed_defined_comorbidity.Rds")
...
write.csv(conmed_smry, "Outputs/comorbidities_overall_before_removing_index.csv")
...
# All index conditions included
setdiff(conmed_all$condition, names(condition_match)) # should be empty
# condition not allocated any matching pairs
condition_match[condition_match == names(condition_match)]# shoudl be empty
# Mismatch between terms for concomitant conditions, should have renal, liver and hypertension only
